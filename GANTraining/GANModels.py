...
class Generator(nn.Module):
    def __init__(self, seq_len=640, patch_size=20, channels=64, num_classes=1, latent_dim=100, embed_dim=10, depth=3,
...
class PatchEmbedding_Linear(nn.Module):
    ...
    def __init__(self, in_channels = 64, patch_size = 20, emb_size = 1280, seq_length = 640):
        super().__init__()
        self.patch_size = patch_size
        ...
        # Calculate number of patches based on input width
        self.num_patches = (seq_length // patch_size)
        self.positions = nn.Parameter(torch.randn(self.num_patches + 1, emb_size))  # +1 for cls token
...
class Discriminator(nn.Sequential):
    def __init__(self, 
        ...
        # dynamically infers emb_size
        if emb_size is None:
          emb_size = in_channels * patch_size
